...
THERON_DECLARE_REGISTERED_MESSAGE(Producer::StartMessage);
THERON_DECLARE_REGISTERED_MESSAGE(EventQueue::StartMessage);
THERON_DECLARE_REGISTERED_MESSAGE(Consumer::StartMessage);

THERON_DEFINE_REGISTERED_MESSAGE(int);
THERON_DEFINE_REGISTERED_MESSAGE(Producer::StartMessage);
THERON_DEFINE_REGISTERED_MESSAGE(EventQueue::StartMessage);
THERON_DEFINE_REGISTERED_MESSAGE(Consumer::StartMessage);
...
int main(int argc, char *argv[])
{
    ...
    printf("Starting %d message passing using Queue\n", numMessages);

    Theron::Framework framework(numThreads);
    Theron::Receiver receiver;


    Producer producer(framework);
    Consumer consumer(framework);
    EventQueue eQueue(framework);

    const Producer::StartMessage prodStart(receiver.GetAddress(), consumer.GetAddress(), eQueue.GetAddress());
    framework.Send(prodStart, receiver.GetAddress(), producer.GetAddress());
    const Consumer::StartMessage consStart(receiver.GetAddress(), producer.GetAddress(), eQueue.GetAddress());
    framework.Send(consStart, receiver.GetAddress(), consumer.GetAddress());
    const EventQueue::StartMessage qStart(receiver.GetAddress(), producer.GetAddress(), consumer.GetAddress());
    framework.Send(qStart, receiver.GetAddress(), eQueue.GetAddress());


    Timer timer;
    timer.Start();

    // Send the initial integer count to Producer
    framework.Send(numMessages, receiver.GetAddress(), producer.GetAddress());
